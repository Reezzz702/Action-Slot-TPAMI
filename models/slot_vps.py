...
class SLOT_VPS(nn.Module):
    def __init__(self, args, num_ego_class, num_actor_class, num_slots=21):
        ...
        # ------------ Slot distributions ------------
        self.slots_mu = nn.Parameter(torch.randn(1, 1, self.hidden_dim2))
        sigma_init = torch.randn(1, 1, self.hidden_dim2).abs()
        self.slots_sigma = nn.Parameter(sigma_init)

        # Keep "slots" buffer for checkpoint compatibility
        dummy_slots = torch.zeros(1, num_slots, self.hidden_dim2)
        self.register_buffer("slots", dummy_slots, persistent=True)
        self.init=False
    ...
    # -----------------------------------------------------------
    # SLOT INITIALIZATION — device safe, runs inside forward()
    # -----------------------------------------------------------
    def initialize_slots(self):
        self.init = True
        device = self.slots_mu.device

        mu = self.slots_mu.expand(1, self.num_slots, -1).to(device)
        sigma = self.slots_sigma.expand(1, self.num_slots, -1).to(device)
    ...
    def forward(self, x):
        ...
        if self.args.backbone == 'inception':
            x = torch.reshape(x, (t*b, 3, h, w))
            x = self.resnet(x)
            _, c, h, w  = x.shape
            x = torch.reshape(x, (self.args.seq_len, b, c, h, w))
        ...
        if self.num_ego_class != 0:
            ...
            ego_x = torch.reshape(ego_x, (b, self.ego_c))
        ...
        # [bs, n, w, h, c]
        x = torch.reshape(x, (b, new_seq_len, new_h, new_w, -1))

        x = self.encoder_pos(x)
        x = torch.reshape(x, (b, -1, self.hidden_dim2))
        # [bs*n, h*w, c]
        x = self.LN(x)
        x = self.fc1(x)
        x = F.relu(x)
        x = self.fc2(x)  
        # [bs, n, h*w, c]
        x = x.view(b, new_seq_len, -1, self.hidden_dim2)

        slots = self.slots.expand(b, new_seq_len, -1, -1)
        ...
        b, t, n, hw = attn_masks.shape
        attn_masks = attn_masks.reshape(b*t, n, -1)
        ...
        attn_masks = attn_masks.view(b, t, n, self.resolution[0], self.resolution[1])
        attn_masks = torch.permute(attn_masks, (0, 2, 1, 3, 4)) # [b, n, t, h, w]
        
        if self.num_ego_class != 0:
            ego_x = self.drop(ego_x)
            ego_x, slots = self.head(slots, ego_x)
            return ego_x, slots, attn_masks, feat_low, feat_high 
        else:
            slots = self.head(slots)
            return slots, attn_masks.view(b, t, n, self.resolution[0], self.resolution[1])

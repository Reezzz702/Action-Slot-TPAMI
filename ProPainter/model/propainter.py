...
class BidirectionalPropagation(nn.Module):
    ...
    def forward(self, x, flows_forward, flows_backward, mask, interpolation='bilinear'):
        ...
        if self.learnable:
            ...
            outputs = self.fuse(torch.cat([outputs_b, outputs_f, mask_in], dim=1)) + x.contiguous().view(-1, c, h, w)
        else:
...
class InpaintGenerator(BaseNetwork):
    ...
    def forward(self, masked_frames, completed_flows, masks_in, masks_updated, num_local_frames, interpolation='bilinear', t_dilation=2):
        ...
        if self.training:
            ...
            mask_pool_l = self.max_pool(ds_mask_in_local.contiguous().view(-1, 1, h, w))
        ...
        if self.training:
            ...
            output = self.decoder(enc_feat[:, :l_t].contiguous().view(-1, c, h, w))

...
def generate_model(args, num_ego_class, num_actor_class, seg=False):
    ...
    if model_name == 'vivit':
        ...
        model = action_slot.ACTION_SLOT(args, num_ego_class, num_actor_class, args.num_slots, box=args.box, seg=seg)
        
        if args.seg_only and not args.ref:
            for t in model.parameters():
                t.requires_grad=False
        ...
        if model_name == 'ARG':
            model = ARG.ARG(args, max_N=20, num_ego_class=num_ego_class, num_actor_class=num_actor_class)
        else:

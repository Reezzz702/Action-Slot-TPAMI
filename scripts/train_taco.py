...
from scripts.generate_model import generate_model
from loss import ActionSlotLoss
from scripts.action_slot_utils import AverageMeter
...
class Engine(object):
	...
	def step(self,batch,mode):
		...
		inputs = batch['videos'].to(self.args.device, dtype=torch.float32) #[b, T, C, h, w]
		...
		if self.args.box:
			...
			if 'slot' in self.args.model_name or 'mvit' in self.args.model_name:
				...
				pred_ego, pred_actor,_,_ = self.model(inputs)
...
if __name__ == '__main__':
	...
	train_set = TACO(args=args, split='train_all')
